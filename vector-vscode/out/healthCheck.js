...
exports.autoConfigureFromReport = autoConfigureFromReport;
/**
 ...
 */
async function runHealthCheck(outputChannel) {
...
async function quickCheck() {
    const report = await (0, envDetector_1.detectEnvironment)();
...
async function showHealthCheckResults(checks, report) {
 ...
 */
async function autoConfigureFromReport(report) {
    const config = vscode.workspace.getConfiguration('vector');
    if (report.pythonPath && !config.get('pythonPath')) {
