...
// ── Core detection ───────────────────────────────────────────────────────────
function execSync(cmd, timeoutMs = 5000) {
    try {
...
function scanForGGUF(dir, models, depth) {
    if (depth > 3) {
        return;
...
function detectMissingDeps(pythonPath) {
    if (!pythonPath) {
        return ['python3'];
    }
    const required = [
...
async function detectEnvironment() {
    ...
    const allGood = (python.path !== null &&
        bestModel !== null &&
